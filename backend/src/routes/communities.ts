...
function mapCommunity(row: Record<string, unknown>) {
  return {
    ...
    image: (row.profile_image_url as string) || (row.cover_image_url as string) || (row.sample_image as string) || null,
    coverImage: (row.cover_image_url as string) || (row.profile_image_url as string) || (row.sample_image as string) || null,
