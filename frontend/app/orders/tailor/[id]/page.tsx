...
type TailorDetail = NonNullable<Awaited<ReturnType<typeof fetchTailorOrderDetail>>>;
  ...
  return (
    <MobileLayout>
      <Box sx={{ flex: 1, display: "flex", flexDirection: "column", bgcolor: "#FAF6F0", minHeight: "100vh" }}>
        <OrderDetailHeader title="ออเดอร์ตัดเสื้อ" />
        <Box sx={{ px: 2, pt: 2, pb: 12 }}>
          ...
          <Box sx={{ bgcolor: "#FFFFFF", borderRadius: "16px", p: 2, mb: 2, border: "1px solid #E5DFD6" }}>
            <Typography sx={{ fontFamily: FONT, fontWeight: 700, fontSize: "0.95rem", color: "#1B2A4A", mb: 1.5 }}>รายละเอียด</Typography>
            <Box sx={{ display: "flex", justifyContent: "space-between", mb: 1 }}>
              <Typography sx={{ fontFamily: FONT, fontSize: "0.85rem", color: "#6B7280" }}>ร้านค้า</Typography>
              <Typography sx={{ fontFamily: FONT, fontSize: "0.85rem", color: "#1B2A4A", fontWeight: 600 }}>{order.shopName ?? "-"}</Typography>
            </Box>
            {order.fabricName && (
              <Box sx={{ display: "flex", justifyContent: "space-between", mb: 1 }}>
                <Typography sx={{ fontFamily: FONT, fontSize: "0.85rem", color: "#6B7280" }}>ผ้าที่ใช้</Typography>
                <Typography sx={{ fontFamily: FONT, fontSize: "0.85rem", color: "#1B2A4A", fontWeight: 600 }}>{order.fabricName} {order.fabricColor ? `(${order.fabricColor})` : ""}</Typography>
              </Box>
            )}
