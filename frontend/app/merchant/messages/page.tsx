    ...
    <Box>
      ...
      {loading ? (
        ...
        <Box sx={{ textAlign: "center", py: 6, color: "#9CA3AF" }}>
          <ChatBubbleOutlineRoundedIcon sx={{ fontSize: 40, mb: 1 }} />
          <Typography sx={{ fontFamily: FONT, fontSize: "0.9rem" }}>ยังไม่มีข้อความจากลูกค้า</Typography>
        </Box>
      ) : (
        <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
          {conversations.map((c) => (
            <Card key={c.id} component={Link} href={`/merchant/messages/${c.id}`}
              sx={{ display: "flex", gap: 1.5, p: 2, border: "1px solid #E5DFD6", borderRadius: "14px", boxShadow: "none", textDecoration: "none", alignItems: "center" }}
            >
