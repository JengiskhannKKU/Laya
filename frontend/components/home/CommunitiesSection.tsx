...
function CommunityCircleCard({ community }: { community: LiveCommunity }) {
  const avatarUrl = SHOP_AVATARS[community.name] || community.image || "/images/trakanta.webp";
  const [imgSrc, setImgSrc] = useState(avatarUrl);

  useEffect(() => {
    setImgSrc(SHOP_AVATARS[community.name] || community.image || "/images/trakanta.webp");
  }, [community]);
